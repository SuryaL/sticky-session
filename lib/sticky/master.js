...
Master.prototype.balance = function balance(socket) {
  let index = 0;
  // if (this.algo == "r") {
  //   var code = .split('').reduce((p, l) => { let c = l.toLowerCase().charCodeAt(0) - 97 + 1; p = p + c; return p; }, '');
  //   var hash = this.hash(code);
  //   index = hash % this.workers.length;
  // } else
  if (this.algo == 'rr') {
    index = this.nextWorker;
    this.nextWorker = this.nextWorker >= this.workerCount - 1 ? 0 : this.nextWorker + 1;
  } else {
